#include<stdio.h>
#include<stdlib.h>
int id_num(char ch)//建表 得到哪個英文會拿到哪個數字
{
    switch(ch)
    {
...
int main()
{
    ...
    tmp=id_num(id[0]);//取英文對應的數字
    total=(tmp%10)*9+tmp/10+(id[1]-'0')*8+(id[2]-'0')*7+(id[3]-'0')*6+(id[4]-'0')*5+(id[5]-'0')*4+(id[6]-'0')*3+(id[7]-'0')*2+(id[8]-'0')*1;//計算P(公式)
    if((10-total%10) ==(id[9]-'0'))//符合
    {
        printf("CORRECT!!!\n");
    }
    else
